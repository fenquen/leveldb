...
// functions.
        ...
        int LockOrUnlock(int fd, bool lock) {
            ...
            fileLockInfo.l_len = 0;  // Lock/unlock entire file.
        ...
        public:
            PosixFileLock(int fd, std::string filename) : fd_(fd),
                                                          filename_(std::move(filename)) {
...
// Instances are thread-safe because all member data is guarded by a mutex.
        ...
        public:
            ...
            Status GetChildren(const std::string &directoryPath, std::vector<std::string> *result) override {
                ...
                if (dir == nullptr) {
                    return PosixError(directoryPath, errno);
                }
            ...
            Status LockFile(const std::string &filename, FileLock **fileLock) override {
                *fileLock = nullptr;
                ...
                if (!locks_.Insert(filename)) {
                    ::close(fd);
                    return Status::IOError("fileLock " + filename, "already held by process");
                }
                ...
                if (LockOrUnlock(fd, true) == -1) {
                    ...
                    return PosixError("fileLock " + filename, lockErrno);
                }

                *fileLock = new PosixFileLock(fd, filename);
...
// Return the maximum number of read-only files to keep open.
    ...
    namespace {
        ...
        using PosixEnvSingleton = SingletonEnv<PosixEnv>;

    }  // namespace

    void EnvPosixTestHelper::SetReadOnlyFDLimit(int limit) {
        PosixEnvSingleton::AssertEnvNotInitialized();
    ...
    Env *Env::Default() {
        static PosixEnvSingleton posixEnvSingleton;
        return posixEnvSingleton.env();
    }
