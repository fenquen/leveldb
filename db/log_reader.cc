    ...
    namespace log {
        ...
        bool Reader::ReadRecord(Slice *dest, std::string *scratch) {
            ...
            while (true) {
                ...
                uint64_t physicalRecordOffset = end_of_buffer_offset_ - buffer_.size() - kHeaderSize - fragment.size();
                ...
                switch (record_type) {
                    ...
                    case kLastType:
                        if (!inFragmentedRecord) {
                            ReportCorruption(fragment.size(),
                                             "missing start of fragmented dest(2)");
                        } else {
                            scratch->append(fragment.data(), fragment.size());
                            *dest = Slice(*scratch);
                            last_record_offset_ = prospectiveRecordOffset;
                    ...
                    default: {
                        char buf[40];
                        std::snprintf(buf, sizeof(buf), "unknown dest type %u", record_type);
                        ReportCorruption(
        ...
        unsigned int Reader::ReadPhysicalRecord(Slice *dest) {
            while (true) {
                ...
                // Parse the header: 4 byte crc + 2 byte length +1 byte len
                ...
                *dest = Slice(header + kHeaderSize, length);
                return type;
            }
        }
