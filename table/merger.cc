...
namespace leveldb {
    ...
    namespace {
        // 把若干个独立的iterator逻辑上变成整体,和netty的bytebuf相似
        class MergingIterator : public Iterator {
        public:
            MergingIterator(const Comparator *comparator,
                            Iterator **children,
                            int n) : comparator_(comparator),
    ...
    Iterator *NewMergingIterator(const Comparator *comparator,
                                 Iterator **iteratorArr,
                                 int iteratorSize) {
        assert(iteratorSize >= 0);
        ...
        return new MergingIterator(comparator, iteratorArr, iteratorSize);
    }
