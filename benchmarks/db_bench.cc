...
// Per-thread state for concurrent executions of the same benchmark.
    ...
    private:
        ...
        void SnappyCompress(ThreadState *thread) {
            RandomGenerator gen;
            Slice input = gen.Generate(Options().blockSize);
            int64_t bytes = 0;
            int64_t produced = 0;
            bool ok = true;
            std::string compressed;
...
int main(int argc, char **argv) {
    FLAGS_write_buffer_size = leveldb::Options().writeBufferSize;
    FLAGS_max_file_size = leveldb::Options().maxFileSize;
    FLAGS_block_size = leveldb::Options().blockSize;
    FLAGS_open_files = leveldb::Options().maxOpenFiles;
